...
async function run() {
    try {
        ...
        if (!user) {
            console.log("No user found in eximclientusers collection!");
            return;
        }
        ...
        const url = "http://localhost:9003/api/transport/data";
        console.log(`\nTESTING URL: ${url}`);
        try {
            ...
            console.log("-> Test SUCCESS!", response.status, response.data);
        } catch (err) {
